...
- **D-01:** Change the objective to **fewest bags primary, cost as tiebreak** — but bounded by a **cost-premium cap**: accept a fewer-bags plan only when its cost ≤ `costMin + price of one smallest available bag` for that color. If the fewer-bags plan exceeds the cap, fall back toward the cost-minimizing plan. (Chosen over pure fewest-bags, which overshoots unboundedly, and over cost-min-primary, which inverts BAG-01's stated priority.)
  - **RESOLUTION (locked during planning, 2026-07-12):** Planning surfaced that D-01's literal *cost* cap contradicts the worked example below (a $0.05 cost premium would wrongly ACCEPT the wasteful `1×2000` for 1050 drills). The developer locked the cap as an **overshoot bound** instead: accept a fewer-bags plan only when its **wasted drills ≤ one smallest available bulk bag's capacity**, else fall back toward the cost-minimizing plan. This reinterprets D-01's literal cost-cap wording to match the worked example (1050 → `1×1000+1×500`, NOT `1×2000`). Consequence recorded for D-06: because the overshoot cap can force the optimizer above the naive single covering bag, the savings figure is **clamped to ≥0** (never negative, never overstated), not provably ≥0.
