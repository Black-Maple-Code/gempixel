...
- [x] 24-02-PLAN.md — MOBILE-02: hand-rolled multi-touch pinch/pan in `viewer.ts` + canvas-only `touch-action:none` + touch-friendly zoom buttons (Wave 1)
...
- [x] 25-02-PLAN.md — Re-token dark viewport to Atelier tokens + bottom-snap the view switcher (D-07/D-09, SC8)
- [x] 25-03-PLAN.md — viewer.ts persistent isFitMode fit-vs-user-zoom flag (D-04 foundation, SC8)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 25-04-PLAN.md — Auto-recompute on dimension change + clean re-fit + rail width-cap + retire manual stale UX/tests (D-02/D-04/D-06, SC5/SC8)
...
- [x] 25-08-PLAN.md — GAP-2 (SC9): pin the Supplies order-summary panel md:sticky (desktop) so it + Next stay in view while the drill list scrolls; mobile keeps natural flow (D-05) [gap_closure]
...
- [x] 26-02-PLAN.md — Re-home: OrderScreen two honest task sections + per-task state; App wires 4 PNG/packet downloads + single Diamond Drills USA cart; tests retargeted (ORDER-04, ORDER-05, D-01/D-03/D-06/D-07) [Wave 2]

**Wave 3** *(blocked on 26-02 — strangler delete)*

- [x] 26-03-PLAN.md — Delete `Step3Canvas.tsx` + `flags.ts` + 2 coupled modals; collapse panel-3 to `SuppliesScreen`; refactor cart-error to the actionError banner; retire coupled tests (ORDER-04, ORDER-05, D-02/D-08) [Wave 3]
