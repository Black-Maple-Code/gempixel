...
---
*Last updated: 2026-07-15 — Milestone v4.0 in progress: Phases 20–24 shipped. Phase 23 assembled the four screens in flow order (Upload → Refine → Supplies → Order) behind the single-mount viewer; Phase 24 validated MOBILE-01/02 — single portrait-column reflow via Tailwind v4 container queries (`@container` on the AtelierShell root + descendant-only `@max-[640px]` overrides, desktop provably unregressed) and hand-rolled multi-touch pinch/pan in `engine/viewer.ts` (an `activePointers` map reusing the cursor-anchored `handleZoom` 0.5–50 clamps, gated on `size===2` for jsdom, with canvas-only `touch-action:none`), plus ≥44px zoom buttons; `npm run build` clean and 355 tests green. A pre-existing StepBar mobile overflow was caught in human-verify and fixed (compact scrollable pills below 640px). Two device-only items (live pinch gesture, Refine sticky-pane render) were user-accepted as proxy-verified. Next: Phase 25 — retire legacy steps + cleanup (milestone close). Backend/fulfillment deferred to v5.0.*
