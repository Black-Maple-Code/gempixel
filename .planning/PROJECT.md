...
---
*Last updated: 2026-07-16 — Milestone v4.0 in progress: Phases 20–25 shipped. Phase 23 assembled the four screens in flow order (Upload → Refine → Supplies → Order) behind the single-mount viewer; Phase 24 validated MOBILE-01/02 (single portrait-column reflow via Tailwind v4 container queries + hand-rolled multi-touch pinch/pan in `engine/viewer.ts`, canvas-only `touch-action:none`, ≥44px zoom buttons). Phase 25 retired the legacy `Step1/2/4` files, re-tokened the canvas viewport off dark slate to Atelier cream, and landed the Phase-24-walkthrough desktop refinements — a fixed 3-zone `AtelierShell` (`h-dvh` top bar + internal-scroll content + pinned Back/Next), a bottom `CanvasControlBar` (view switcher + zoom, moved off the raster), auto-recompute-and-refit on SizeCard change (no manual Recompute), and plain-Ctrl+P print-the-grid-from-any-step; UAT Test 29 re-verified (human-approved), `npm run build` clean, 373 tests green. The still-live Diamond Drills USA drill-cart handoff + canvas/legend PNG export were deliberately preserved (dormant behind the shipped `USE_NEW_*` screens) for Phase 26; the advisory code review (0 critical / 5 warning) flagged WR-01 (those very exports being unreachable) as the Phase-26 re-home scope, and its actionable warnings — WR-02 (debounced custom-size recompute not cancelled on select/reset/load), WR-04 (dark-slate remnants in dormant modals + error banners), WR-05 (unused `isFitMode` API) — plus a short-viewport Refine canvas-fit nuance were logged as four Phase-26 todos. Next: Phase 26 — interim customer fulfillment (re-home the canvas-PNG packet + Diamond Drills USA order). Backend/fulfillment (payments, lab submission) deferred to v5.0.*
